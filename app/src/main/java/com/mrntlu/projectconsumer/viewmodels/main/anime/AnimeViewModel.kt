...
class AnimeViewModel @AssistedInject constructor(
    ...
    init {
        if (page != 1) {
            ...
            startAnimeFetch(true)
        }
    }

    fun startAnimeFetch(refreshAnyway: Boolean = false) {
        if (refreshAnyway)
            setPagePosition(1)
    ...
    fun fetchAnime() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            animeRepository.fetchAnimes(page, getSort(), tag, isNetworkAvailable).collect { response ->
    ...
    private fun restoreData() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            animeRepository.fetchAnimes(page, getSort(), tag, isNetworkAvailable, isRestoringData = true).collect { response ->
                if (response.isSuccessful()) {
